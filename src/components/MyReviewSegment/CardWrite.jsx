...
const CardWrite = ({reviewData}) => {
  return (
    <ul className="px-5">
      {reviewData.map((review, index) => (
        <li className="flex items-center py-5 border-b border-black-200" key={index}>
          <div
          ...
          <ul className="grow">
            <li className="text-xs flex flex-col justify-center gap-2">
              ...
              <p className="">{review.product_name}</p>
              <p className="text-black-600">옵션: {review.product_option}</p>
            </li>
          </ul>
          <button className="bg-point text-white w-[5.188rem] rounded p-0.5">
            리뷰 작성
          </button>
        </li>
      ))}
